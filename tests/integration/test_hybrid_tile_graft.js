...
(async () => {
    ...
    const capabilityIds = ["map:read", "map:write", "world:read", "world:write", "project:read", "project:write", "project:validate", "recipes:read", "recipes:write", "pack:manage", "ui:contribute", "network", "process", "clipboard"];
    const capabilityReport = api.validateExtensionManifest({ id: "test.capabilities", name: "Capability contract", version: "1.0.0", permissions: capabilityIds, contributes: {} });
    assert.strictEqual(capabilityReport.ok, true); assert.deepStrictEqual(capabilityReport.permissions, capabilityIds);
    ...
    const policyV16 = api.setExtensionCapabilityPolicyV16("test.worldstudio", { capabilities: ["map:read", "world:read", "ui:contribute", "network", "unknown"], isolation: "worker" }); assert.deepStrictEqual(policyV16.capabilities, ["map:read", "world:read", "ui:contribute", "network"]); assert.deepStrictEqual(policyV16.denied, ["unknown"]);
    ...
    // Every Plugin Manager command publishes a machine-readable outcome and can drive event variables/switches.
    const commandInterpreter = new Game_Interpreter(); const observedCommandResults = []; const unsubscribeCommandResults = api.onCommandResult(result => observedCommandResults.push(result));
    PluginManager.commands["HybridTileGraft:systemHealth"].call(commandInterpreter, { resultVariableId: "20", successSwitchId: "21", failureSwitchId: "22" });
    const successfulCommand = api.lastCommandResult(); assert.strictEqual(successfulCommand.command, "systemHealth"); assert.strictEqual(successfulCommand.status, "succeeded"); assert.strictEqual(successfulCommand.ok, true); assert.strictEqual(global.$gameSwitches.value(21), true); assert.strictEqual(global.$gameSwitches.value(22), false); assert.strictEqual(JSON.parse(global.$gameVariables.value(20)).sequence, successfulCommand.sequence);
    assert.throws(() => PluginManager.commands["HybridTileGraft:setExtensionCapabilityPolicyV16"].call(commandInterpreter, { extensionId: "", policyJson: "{}", resultVariableId: "20", successSwitchId: "21", failureSwitchId: "22", failureCommonEventId: "23" }), /extension id/i);
    const failedCommand = api.lastCommandResult(); assert.strictEqual(failedCommand.command, "setExtensionCapabilityPolicyV16"); assert.strictEqual(failedCommand.status, "failed"); assert.strictEqual(failedCommand.ok, false); assert.match(failedCommand.error.message, /extension id/i); assert.strictEqual(global.$gameSwitches.value(21), false); assert.strictEqual(global.$gameSwitches.value(22), true); assert.strictEqual(global.$gameTemp.reserved, 23); assert.ok(observedCommandResults.length >= 2); assert.strictEqual(unsubscribeCommandResults(), true); assert.strictEqual(api.clearCommandResult().sequence, failedCommand.sequence); assert.strictEqual(api.lastCommandResult(), null);
